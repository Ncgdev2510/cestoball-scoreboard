...
export default function ControlPanel() {
  ...
  return (
    <div className="min-h-screen bg-brand-dark text-white flex flex-col overflow-x-hidden">
      ...
      {toast && (
        <div
          className={`fixed top-4 right-4 z-50 max-w-sm rounded-xl px-4 py-3 text-sm shadow-2xl transition-all duration-300 ${
            ...
            toast.type === 'success'
              ? 'border border-brand-steel/50 bg-brand-card text-brand-rose'
              : toast.type === 'error'
                ? 'border border-brand-bronze/60 bg-brand-dark text-brand-terracotta'
                : 'border border-brand-steel/40 bg-brand-surface text-brand-rose'
          }`}
        >
          {toast.message}
        </div>
      ...
      {/* Header */}
      <header className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-b border-brand-steel/20 bg-brand-card shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-xs text-brand-steel">Panel de Control</span>
          </div>
        </div>
        ...
        <input
          ...
          className="bg-brand-surface/40 border-b border-brand-steel/30 text-center text-sm text-brand-rose focus:outline-none focus:border-brand-steel transition-colors px-2 py-1 w-44 md:w-56 rounded-t"
        />
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-brand-steel hover:bg-brand-steel/90 text-white text-xs font-bold px-3 py-2 rounded-lg shadow-lg shadow-brand-steel/30 transition-all active:scale-95"
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-brand-surface hover:bg-brand-petrol text-brand-rose border border-brand-steel/30 text-xs font-semibold px-2.5 py-2 rounded-lg transition-all active:scale-95"
          ...
          <button
            ...
            className={`flex items-center gap-1.5 text-xs font-semibold px-2.5 py-2 rounded-lg transition-all active:scale-95 ${
              keyboardLocked
                ...
                : 'bg-brand-surface hover:bg-brand-petrol text-brand-rose border border-brand-steel/30'
            }`}
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-brand-petrol hover:bg-brand-steel text-white text-xs font-semibold px-3 py-2 rounded-lg transition-all active:scale-95 border border-brand-steel/30 shadow-md"
          >
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-[1fr_1.4fr_1fr] gap-0 overflow-y-auto lg:overflow-hidden">
        {/* Left: Home team */}
        <div className="flex flex-col gap-6 p-5 lg:p-8 border-b lg:border-b-0 lg:border-r border-brand-steel/20 bg-gradient-to-b from-brand-petrol/20 to-brand-dark">
          ...
          <div className="mt-auto">
            <div className="flex flex-col gap-3">
              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-brand-steel">Acciones</span>
              <ExtrasControl />
            </div>
          </div>
        </div>
        ...
        {/* Center: Clock & Alarms */}
        <div className="flex flex-col gap-6 p-5 lg:p-8 border-b lg:border-b-0 lg:border-r border-brand-steel/20 bg-brand-dark">
          <div className="flex-1 flex flex-col items-center justify-center">
            <ClockControl />
          </div>
          
          <div className="mt-auto border-t border-brand-steel/20 pt-6">
            <div className="flex flex-col gap-3 max-w-2xl mx-auto w-full">
              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-brand-steel">Alarma</span>
              <AlarmControl />
            </div>
          </div>
        </div>
        ...
        {/* Right: Away team */}
        <div className="flex flex-col gap-6 p-5 lg:p-8 bg-gradient-to-b from-brand-bronze/15 to-brand-dark">
          ...
          <div className="mt-auto">
            <div className="flex flex-col gap-3">
              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-brand-terracotta">Atajos</span>
              <div className="rounded-xl border border-brand-steel/20 bg-brand-card px-4 py-3 text-xs text-brand-rose">
                <div className="flex items-center gap-2 text-white font-semibold mb-2">
                  <Keyboard size={14} className="text-brand-steel" /> Atajos Teclado
                </div>
